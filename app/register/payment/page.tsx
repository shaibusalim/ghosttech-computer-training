  ...
  const handleMomoPayment = async () => {
    ...
    try {
      ...
      const resp = await fetch("/api/payments/paystack/initialize", {
        ...
        // We'll pass the momo details to the backend to initialize the charge
        body: JSON.stringify({ 
          registrationId, 
      ...
      // If we used the standard initialize API, we'll get an authorization_url
      // but for the user's specific request "get prompt on phone", we can either 
      // 1. Redirect to Paystack which handles the prompt
      // 2. Or use Paystack Charge API directly on the backend.
      // Let's stick with redirecting for now as it's more robust for all providers,
      // but we'll use the Paystack checkout to handle it.
      
      if (json.authorization_url) {
        window.location.href = json.authorization_url as string
      } else {
        // Fallback for direct charge success
        router.push(`/register/payment/verify?registrationId=${registrationId}&reference=${json.reference}`)
      }
